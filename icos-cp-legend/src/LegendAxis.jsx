...
export default class LegendAxis extends Component {
	constructor(props) {
		...
		if (props.useRangeValueFilters) {
			...
			this.svg = undefined;
			this.maxRangeSlider = undefined;
			this.minRangeSlider = undefined;
	...
	componentDidMount() {
		if (this.props.useRangeValueFilters) {
			this.svg = document.getElementById("svg");
			this.maxRangeSlider = document.getElementById("maxRange");
			this.minRangeSlider = document.getElementById("minRange");
	...
	updateRangeFilter(){
		if (this.props.rangeFilterChanged) {
			...
			if (this.transform.matrix.f === 0){
				this.props.rangeFilterChanged({[id]: undefined});
				...
				this.props.rangeFilterChanged({[id]: dataVal});
			}
		}
	}

	setRangeValuePosition(rangeValuesFromProps){
		const {minRange, maxRange} = rangeValuesFromProps;
		const {length, pixelMaker} = this.props;
		const {minRangeSlider, maxRangeSlider, svg} = this;

		if (minRangeSlider && maxRangeSlider && svg) {
			const transformsMin = minRangeSlider.transform.baseVal;
			const transformsMax = maxRangeSlider.transform.baseVal;

			if (transformsMin.length === 0 || transformsMin.getItem(0).type !== SVGTransform.SVG_TRANSFORM_TRANSLATE) {
				const translate = svg.createSVGTransform();
				translate.setTranslate(0, 0);
				minRangeSlider.transform.baseVal.insertItemBefore(translate, 0);
			}

			if (transformsMax.length === 0 || transformsMax.getItem(0).type !== SVGTransform.SVG_TRANSFORM_TRANSLATE) {
				const translate = svg.createSVGTransform();
				translate.setTranslate(0, 0);
				maxRangeSlider.transform.baseVal.insertItemBefore(translate, 0);
			}

			if (minRange === undefined) {
				transformsMin.getItem(0).setTranslate(0, 0);

			} else {
				const ty = -pixelMaker(minRange);
				transformsMin.getItem(0).setTranslate(0, ty);
			}

			if (maxRange === undefined) {
				transformsMax.getItem(0).setTranslate(0, 0);

			} else {
				const ty = length - 1 - pixelMaker(maxRange);
				transformsMax.getItem(0).setTranslate(0, ty);
			}
		}
	}
	...
	render() {
		...
		const decimals = props.decimals
			? props.decimals
			: 0;
		this.setRangeValuePosition(props.rangeValues);
...
const MaxRange = ({useRangeValueFilters, horizontal, margin, width}) => {
	if (!useRangeValueFilters || horizontal) return null;
...
const MinRange = ({useRangeValueFilters, horizontal, margin, width, height}) => {
	if (!useRangeValueFilters || horizontal) return null;
