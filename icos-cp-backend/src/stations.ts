export type Modifiers = Partial<StationsDict<(v: string, row: StationsDict<BindingEntry, VarName>) => string, VarName>>
export type ParsedSparqlValue = number | Date | string
export type TransformPointFn = (lonOrX: number, latOrY: number) => [number, number]
export type ReducedStation = Partial<StationsDict<ParsedSparqlValue | [number, number] | boolean, VarName | 'type' | 'point' | 'Short_name' | 'id' | 'hasLandingPage'>>

export class StationParser {
	constructor(private readonly countries: StationsDict, private readonly transformPointFn?: TransformPointFn) { }

	parse(bindings: StationsDict<BindingEntry, VarName>[]) {
		return this.transformPointFn === undefined
			? this.parseToRowColArrays(bindings)
			: this.parseToObjectArray(bindings);
	}

	private parseToRowColArrays(bindings: StationsDict<BindingEntry, VarName>[]) {
	...
	private parseToObjectArray(bindings: StationsDict<BindingEntry, VarName>[]) {
		const localModifiers = {
			[Vars.stationId]: (v: string, row: StationsDict<BindingEntry, VarName>) => (row[Vars.prodUri]
				? row[Vars.prodUri].value
				: v
			),
			[Vars.labelingDate]: (v: string, row: StationsDict<BindingEntry, VarName>) => (row[Vars.labelingDate]
	...
	private commonModifiers(countries: StationsDict): Modifiers {
		return {
			[Vars.theme]: (v: string, row: StationsDict<BindingEntry, VarName>) => themeName[row[Vars.themeShort].value as ThemeShort] ?? "?",
	...
	private sparqlBindingToValue(b: StationsDict) {
...
export type StationsDict<Value = string, Keys extends string | number | symbol = string> = Record<Keys, Value>
export type BindingEntry = StationsDict<string, 'type' | 'value' | 'datatype'>
...
export type SparqResp = {
	...
	results: {
		bindings: StationsDict<BindingEntry, VarName>[]
	}
}
...
export const themeName: StationsDict<string, ThemeShort> = { AS: "Atmosphere", ES: "Ecosystem", OS: "Ocean" };
...
const mergeProvAndProd = ([prov, prod]: [SparqResp, SparqResp]) => {
	const prodLookup = prod.results.bindings.reduce<StationsDict<StationsDict<BindingEntry, VarName>>>((acc, next) => {
